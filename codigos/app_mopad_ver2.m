    ...
    methods (Access = private)
        ...
        if app.n~=0
            ...
            if ~isempty(app.home)
                ...
                fprintf(fileID,'%.2f  \t%.2f  \t%.2f  \t%s  \t%i  \t%s  \t%f  \t%s  \t%s  \t%i  \n',app.home,'home',0,'home',0.95,'home','home',0);
            end
        ...
        function HOMEButtonPushed(app, event)
            %Establece un HOME para retorno del robot
                ...
                app.home_y = round(-(pos_map.Pose.Position.Y-app.origin(2))/app.resolution++size(app.mapa,1));
